...
class DWA:
    def __init__(self, publisher):
        ...
        self.status_pub = rospy.Publisher('dwa_status', Bool, queue_size=1)
        self.status = 'wait'
    ...
    def lds_callback(self, lds):
        ...
        if self.status == 'dwa_turn':
            self.dwa_status = True
            self.status_pub.publish(self.dwa_status)
            ...
            while self.current_angle < self.target_angle:
                ...
                self.current_angle = (self.current_time - self.start_time).to_sec() * self.target_angle.angular.z
        ...
        if self.status == 'go_goal':
            ...
            self.publisher.publish(turtle_vel)
            print(f"turtle vel : {turtle_vel.linear.x}, {turtle_vel.angular.z}111")
...
def main():
    ...
    rospy.Subscriber('status_pub', String, driver.status_callback)
